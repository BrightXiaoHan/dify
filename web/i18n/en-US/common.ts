const translation = {
  ...
  account: {
    ...
    langGeniusAccount: '法治评估 账号',
    langGeniusAccountTip: '您的 法治评估 账号和相关的用户数据。',
    editName: '编辑名字',
  ...
  members: {
    ...
    invitationSentTip: '邀请已发送，对方登录 法治评估 后即可访问你的团队数据。',
    invitationLink: '邀请链接',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '体验',
      exhausted: '超出限额',
      desc: '托管 OpenAI 由 法治评估 提供的托管 OpenAI 服务，你可以使用 GPT-3.5 等模型，在体验额度消耗完毕前你需要设置其它模型供应商。',
      callTimes: '调用次数',
  ...
  modelProvider: {
    ...
    rerankModel: {
      key: 'Rerank 模型',
      tip: '重排序模型将根据候选文档列表与用户问题法治评估匹配度进行重新排序，从而改进法治评估排序的结果',
    },
    quota: '额度',
    searchModel: '搜索模型',
  ...
  apiBasedExtension: {
    title: 'API 扩展提供了一个集中式的 API 管理，在此统一添加 API 配置后，方便在 法治评估 上的各类应用中直接使用。',
    add: '新增 API 扩展',
    selector: {
      title: 'API 扩展',
  ...
  about: {
    changeLog: '更新日志',
    updateNow: '现在更新',
    nowAvailable: '法治评估 {{version}} 现已可用。',
    latestAvailable: '法治评估 {{version}} 已是最新版本。',
  },
  appMenus: {
    overview: '监测',
    promptEng: '编排',
